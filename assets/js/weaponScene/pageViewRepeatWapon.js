...
cc.Class({
    ...
    start() {
        ...
        this.speed = 0;
        this.pageIndexNow = globalData.gunType;
    ...
    btnMove(_left) {
        ...
        for (var i = 0; i < this.itemCount; i++) {
            ...
            this.itemArr[i].getChildByName("yellow").active = false;
    ...
    initTouch() {
        ...
        this.node.on(cc.Node.EventType.TOUCH_MOVE, (event) => {
            ...
            this._offset += event.getDelta().y;

            var _x = event.getDelta().y;
    ...
    huiGui() {
        ...
        this.scheduleOnce(() => {
            ...
            cc.director.emit("切换到枪支的index", this.pageIndexNow);
            _targetItem.getChildByName("yellow").active = true;
            AD.audioMng.playSfx("切换")
        }, _duration)
    },
    update(dt) {
        ...
        // if (this.guanXing == false) 
        {
            ...
            for (var i = 0; i < this.itemCount; i++) {
                ...
                var _scale = (this.viewW * 2.5 - Math.abs(this.itemArr[i].x)) / (this.viewW * 2.5);
                if (_scale < 0)
                    _scale = 0;
                else {
                ...
                this.itemArr[i].y = -(_scale-0.76)*150;
            }
